...
<required_reading>
@~/.claude/phaseflow/references/verification-overrides.md
...
@~/.claude/phaseflow/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/phaseflow/references/few-shot-examples/verifier.md
...
phase-sdk query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(phase-sdk query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `phase-sdk query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(phase-sdk query verify.artifacts "$PLAN_PATH")
```
...
Use `phase-sdk query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(phase-sdk query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(phase-sdk query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(phase-sdk query verify.commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(phase-sdk query roadmap.analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/phase:plan-phase --gaps`:
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@~/.claude/phaseflow/references/verify-mvp-mode.md`.
...
```bash
USER_STORY_VALID=$(phase-sdk query user-story.validate --story "$PHASE_GOAL" --pick valid)
```

If `valid != true`, refuse to verify. Surface the discrepancy and ask the user to run `/phase mvp-phase ${PHASE}` to set a proper User Story goal. The verb owns the canonical regex `/^As a .+, I want to .+, so that .+\.$/` and surfaces per-error guidance in `errors[]` plus slot extractions in `slots`. Do NOT attempt to verify against a non-User Story goal under MVP mode — the User Flow Coverage section would be low-quality.
...
_Verifier: Claude (phase-verifier)_
```
...
Structured gaps in VERIFICATION.md frontmatter for `/phase:plan-phase --gaps`.
