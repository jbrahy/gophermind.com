---
name: phase:config
description: Configure PhaseFlow settings — workflow toggles, advanced knobs, integrations, and model profile
...
<objective>
Configure PhaseFlow settings interactively with a single consolidated command.
...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | phase-sdk config-set-model-profile |

</routing>

<execution_context>
@~/.claude/phaseflow/workflows/settings.md
@~/.claude/phaseflow/workflows/settings-advanced.md
@~/.claude/phaseflow/workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. **Pre-flight check (#2439):** verify `phase-sdk` is on PATH via `command -v phase-sdk`.
     If absent, emit the install hint `Install PhaseFlow via 'npm i -g phaseflow'` and stop —
     do NOT invoke `phase-sdk` directly (avoids the opaque `command not found: phase-sdk` failure).
  2. Run: `phase-sdk query config-set-model-profile <profile-name> --raw` and display the output verbatim.
