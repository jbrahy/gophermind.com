...
4. Delegates to `/phase plan-phase <N>` which auto-detects MVP mode via the roadmap field

Phase 1 of the vertical-mvp-slice PRD shipped the planner-side machinery; this command is the user entry point for it.
</objective>

<execution_context>
@~/.claude/phaseflow/workflows/mvp-phase.md
@~/.claude/phaseflow/references/spidr-splitting.md
@~/.claude/phaseflow/references/user-story-template.md
</execution_context>
...
The phase must already exist in ROADMAP.md (created via `/phase new-project`, `/phase add-phase`, or `/phase insert-phase`). This command does not create new phases — it converts an existing phase to MVP mode.
</context>

<process>
Execute the mvp-phase workflow from @~/.claude/phaseflow/workflows/mvp-phase.md end-to-end.
