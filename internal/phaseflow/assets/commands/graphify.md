...
**CJS-only (graphify):** `graphify` subcommands are not registered on `phase-sdk query`. Use `node $HOME/.claude/phaseflow/bin/phase-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `phase-sdk query` where a handler exists.
...
**DO NOT use the phase-tools config get-value command** -- it hard-exits on missing keys.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude/phaseflow/bin/phase-tools.cjs config-set graphify.enabled true

Then run /phase:graphify build to create the initial graph.
```
...
```bash
node $HOME/.claude/phaseflow/bin/phase-tools.cjs graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /phase:graphify build to create or rebuild the graph.`
...
```bash
node $HOME/.claude/phaseflow/bin/phase-tools.cjs graphify diff
```
...
```bash
node "$HOME/.claude/phaseflow/bin/phase-tools.cjs" graphify build
```
...
```text
PhaseFlow > Building knowledge graph...
```
...
graphify update . \
  ...
  && node "$HOME/.claude/phaseflow/bin/phase-tools.cjs" graphify build snapshot \
  && node "$HOME/.claude/phaseflow/bin/phase-tools.cjs" graphify status
```
...
**MVP-mode rendering.** When a phase has `**Mode:** mvp` in ROADMAP.md (resolved via `phase-sdk query roadmap.get-phase --pick mode`), render its graph node with two distinct visual signals:
...
5. DO NOT use `phase-tools config get-value` for the config gate -- it exits on missing keys.
