---
name: phase:workspace
description: Manage PhaseFlow workspaces — create, list, or remove isolated workspace environments
...
<objective>
Manage PhaseFlow workspaces with a single consolidated command.

Mode routing:
- **--new**: Create an isolated workspace with repo copies and independent .planning/ → new-workspace workflow
- **--list**: List active PhaseFlow workspaces and their status → list-workspaces workflow
- **--remove**: Remove a PhaseFlow workspace and clean up worktrees → remove-workspace workflow
</objective>
...
| --list | Scan ~/phase-workspaces/, show summary table | list-workspaces |
| --remove | Confirm and remove workspace directory | remove-workspace |

</routing>

<execution_context>
@~/.claude/phaseflow/workflows/new-workspace.md
@~/.claude/phaseflow/workflows/list-workspaces.md
@~/.claude/phaseflow/workflows/remove-workspace.md
