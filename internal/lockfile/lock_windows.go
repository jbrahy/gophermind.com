...
// staleLockAge is how old a lock file's mtime must be before Acquire treats
...
// this takeover a process that dies mid-write would wedge the lock for
// every future caller, forever. 60 seconds is comfortably longer than a
// single locked write (a read-modify-write of a small JSON file) can
// legitimately take, so a lock file older than that is treated as abandoned
// rather than contended.
const staleLockAge = 60 * time.Second

// Acquire emulates an exclusive lock with an atomically created lock file,
...
// crash cannot wedge the lock permanently.
func Acquire(path string) (func(), error) {
	...
	for {
		...
		if time.Now().After(deadline) {
			return nil, fmt.Errorf("lockfile: lock busy: %w", err)
		}
