...
- **Free LLM providers** — a vendored, CC0 registry of 16 free OpenAI-compatible providers and 118 models (from `mnfst/awesome-free-llm-apis`), selectable as `free-*` profiles. The model in use is named with a link to its provider, and any free-tier obligation worth knowing about (non-commercial only, trains on your prompts, identity check) is surfaced rather than buried.
- **Usage odometer** — a lifetime, monotonic count of free tokens and requests, plus rolling trip meters per published quota window. Recorded per model, not just per provider. Where a provider publishes no rate limit, none is shown: a guessed denominator reads exactly like a real one and is worse than no number at all.
- **Desktop application** — a Wails shell around the harness, with a chat screen, an approvals screen, a model picker and a settings panel. The frontend speaks only HTTP to an embedded instance of the same server the CLI runs; there is exactly one native binding, which tells the frontend where that server is.
...
- **`run_shell` no longer passes `GIT_*` variables to the commands it runs.** An inherited `GIT_DIR` redirects a git command away from the directory it was pointed at and onto whatever repository that variable names — the failure that once produced commits deleting 803 files and left `core.bare` set on a working repo. `internal/gitenv` had fixed this for git commands the codebase builds itself, but `run_shell` is the path an agent actually runs `git init` and `git commit` through. Both shell tools are fixed, and the model-supplied `env_allow_list` is filtered too, so naming `GIT_DIR` there cannot reinstate it.
...
- **Every turn is metered, not only the CLI's one-shot path.** Turns served over HTTP, through a session, or in the desktop app spent free-tier allowance that no counter saw, so the desktop model picker's usage figures stayed at full forever and cycle-on-capacity could never fire.
...
- **A contract flag stops the run**, rather than only the current pass, and is rendered as itself instead of as a success.
- **A corrupt settings file is reported** instead of silently yielding defaults, which re-enabled providers excluded for legal reasons; a failed odometer read no longer overwrites the lifetime reading with zeros.
