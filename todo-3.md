...
- [x] **4. Stdin control messages.** Accept `system`/control lines (interrupt, set-model) on the stream-json input. Value: mid-session control from the driver.
...
- [x] **12. Session branching.** Fork a session at a turn into a new id. Value: explore alternatives without losing state.
- [x] **13. Session TTL / GC.** Auto-expire old sessions. Value: bounded disk footprint.
- [x] **14. Named sessions.** Human aliases mapping to ids (`--resume my-refactor`). Value: no uuids to remember.
...
- [x] **17. Interactive resume in the TUI.** Pick a saved session at chat startup. Value: continuity in interactive mode.
...
- [x] **31. Build the structured prompt system.** Ship `PLAN.md`: `internal/prompt` template parser + builder (YAML frontmatter + XML sections). Value: composable, maintainable prompts.
...
- [x] **35. Prompt fragments / includes.** Reusable snippets composed into the prompt. Value: DRY prompt maintenance.
...
- [x] **38. A/B prompt experiments.** Run variants against fixtures and score. Value: data-driven prompt tuning.
- [x] **39. Skill files.** Discover `.gophermind/skills/*.md` and inject relevant ones. Value: modular capability packs.
...
- [x] **70. Offline docs cache.** Persist fetched docs for reuse offline. Value: speed + air-gapped support.
