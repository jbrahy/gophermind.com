...
- [x] **17. Parquet & Arrow support.** Teach `inspect_data`/`data_transform` to read columnar formats. Value: modern data-lake files.
...
- [x] **22. Planner/executor split.** A dedicated planner agent emits a task graph; executor agents (fleet) run leaves in dependency order. Value: parallelism with correct ordering.
...
- [x] **45. Shared session store.** Optional remote (S3/HTTP) backend for the session dir so a team shares sessions. Value: hand off work across machines.
...
- [x] **71. OpenTelemetry tracing.** Emit spans for turns, tool calls, and LLM requests. Value: end-to-end latency/why-is-it-slow visibility.
...
- [x] **81. Language Server (LSP) client.** Use a project's LSP for go-to-def/references instead of grep (upgrades `find_symbol`). Value: precise, semantic navigation.
...
- [x] **83. Editor plugins.** Thin VS Code / Neovim clients that drive `--print` stream-json. Value: gophermind inside the editor.
...
- [x] **92. Tool marketplace/registry.** Discover and install community tools/skills by name. Value: capabilities without recompiling.
...
- [x] **94. WASM tool sandbox.** Run untrusted community tools in a WASM runtime with capability grants. Value: safe third-party extensions.
...
- [x] **97. `gophermind upgrade`.** Signed self-update from GitHub Releases (fulfills batch-3 U#26 with cosign verification). Value: stay current safely.
