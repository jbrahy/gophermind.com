...
- [ ] **2. Pluggable embeddings provider.** An `internal/embed` interface with OpenAI-compatible `/v1/embeddings` and a local fallback. Value: the vector store works against any endpoint the LLM client already talks to.
...
- [ ] **5. Web-search result re-ranking.** Score `web_search` hits by embedding similarity to the query before returning. Value: the top result is actually the most relevant, not just Brave's default order.
...
- [ ] **12. Schema explorer tool.** A `db_schema` tool returning tables, columns, types, and FKs as structured JSON. Value: the model orients before querying.
- [ ] **13. Query plan / EXPLAIN helper.** Surface `EXPLAIN QUERY PLAN` and flag full-table scans. Value: performance-aware SQL suggestions.
...
- [ ] **20. Query result caching.** Cache `sql_query` results by (db-mtime, query, params) hash. Value: instant repeats during analysis.
...
- [ ] **24. Self-consistency sampling.** Run N samples of a turn and majority-vote the answer. Value: robustness on reasoning tasks.
...
- [ ] **30. Deterministic replay tests.** Record a session's LLM responses and replay them offline to test agent logic. Value: fast, hermetic agent tests.

## AF. Prompt Engineering & Evaluation II (builds on `internal/prompt`, `internal/abtest`, `--persona`, skills)

- [ ] **31. Prompt registry & versioning.** Store named prompt templates with versions and a `prompts` subcommand to list/diff/rollback. Value: manage prompts like code.
...
- [ ] **42. Session merge.** Combine two branched sessions' histories with conflict handling. Value: reconcile parallel explorations.
- [ ] **43. Session search.** Full-text (FTS5) search across all saved sessions. Value: "where did we discuss the parser bug?"
...
- [ ] **47. Redacted session export.** Combine `export` with the existing redactor to share a scrubbed session. Value: safe sharing of debugging sessions.
- [ ] **48. Session tags & filters.** Tag sessions and filter `sessions list` by tag/date/repo. Value: organize a growing store.
...
- [ ] **55. Webhook signature verification.** HMAC-verify inbound `serve` payloads (GitHub/Stripe style). Value: trust the trigger source, not just the bearer token.
...
- [ ] **73. Cost dashboard.** A `usage report` subcommand summarizing spend by day/model/session. Value: understand where budget goes.
...
- [ ] **76. Structured JSON logs.** Optional machine-readable logs for every event. Value: pipe into log tooling.
- [ ] **77. Slow-request tracing.** Flag and record LLM/tool calls over a latency threshold. Value: find the bottleneck.
- [ ] **78. Health/readiness endpoints.** `/healthz` and `/readyz` on `serve`. Value: deploy behind a load balancer / k8s.
