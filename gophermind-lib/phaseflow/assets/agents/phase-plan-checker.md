...
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /phase:plan-phase orchestrator.
...
Spawned by `/phase:plan-phase` orchestrator (after planner creates PLAN.md) or re-verification (after planner revises).
...
<required_reading>
@~/.claude/phaseflow/references/gates.md
</required_reading>
...
<upstream_input>
**CONTEXT.md** (if exists) — User decisions from `/phase:discuss-phase`
...
**The difference:**
- `phase-verifier`: Verifies code DID achieve goal (after execution)
- `phase-plan-checker`: Verifies plans WILL achieve goal (before execution)
...
@~/.claude/phaseflow/references/few-shot-examples/plan-checker.md
...
**Question:** Do plans honor user decisions from /phase:discuss-phase?
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/phase:plan-phase {N} --research` to regenerate."
...
```bash
INIT=$(phase-sdk query init.phase-op "${PHASE_ARG}")
...
```bash
phase-sdk query phase.list-plans "$phase_number"
# Research / brief artifacts (deterministic listing)
phase-sdk query phase.list-artifacts "$phase_number" --type research
phase-sdk query roadmap.get-phase "$phase_number"
phase-sdk query phase.list-artifacts "$phase_number" --type summary
```
...
Use `phase-sdk query` to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(phase-sdk query verify.plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using `phase-sdk query`:

```bash
MUST_HAVES=$(phase-sdk query frontmatter.get "$PLAN_PATH" must_haves)
```
...
```bash
PLAN_STRUCTURE=$(phase-sdk query verify.plan-structure "$PLAN_PATH")
```
...
```bash
phase-sdk query plan.task-structure "$PLAN_PATH"
```
...
```bash
phase-sdk query plan.task-structure "$PHASE_DIR/$PHASE-01-PLAN.md"
phase-sdk query frontmatter.get "$PHASE_DIR/$PHASE-01-PLAN.md" files_modified
```
...
Plans verified. Run `/phase:execute-phase {phase}` to proceed.
```
...
**DO NOT** check code existence — that's phase-verifier's job. You verify plans, not codebase.
