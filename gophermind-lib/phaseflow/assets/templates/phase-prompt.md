...
> **Note:** Planning methodology is in `agents/phase-planner.md`.
...
@~/.claude/phaseflow/references/checkpoints.md
</execution_context>
...
<!-- For checkpoint task examples and patterns, see @~/.claude/phaseflow/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/phase:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/phaseflow/references/tdd.md` for TDD plan structure.
...
See `~/.claude/phaseflow/templates/user-setup.md` for full schema and examples
...
See `~/.claude/phaseflow/workflows/verify-phase.md` for verification logic.
