...
**Mode routing:**
```bash
DISCUSS_MODE=$(phase-sdk query config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```

If `--assumptions` is in $ARGUMENTS:
Read and execute `~/.claude/phaseflow/workflows/list-phase-assumptions.md` end-to-end.
Stop here.

Otherwise, if `DISCUSS_MODE` is `"assumptions"`:
Read and execute `~/.claude/phaseflow/workflows/discuss-phase-assumptions.md` end-to-end.

Otherwise (`"discuss"` / unset / any other value):
Read and execute `~/.claude/phaseflow/workflows/discuss-phase.md` end-to-end.
