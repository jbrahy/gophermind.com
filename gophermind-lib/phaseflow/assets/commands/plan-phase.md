...
**Research-only mode (`--research-phase <N>`):** Spawn `phase-phase-researcher` for phase `N`, write `RESEARCH.md`, then exit before the planner runs. Useful for cross-phase research, doc review before committing to a planning approach, and correction-without-replanning loops where iterating on research alone is dramatically cheaper than re-spawning the planner. Replaces the deleted research-phase command (#3042).
...
**Orchestrator role:** Parse arguments, validate phase, research domain (unless skipped), spawn phase-planner, verify with phase-plan-checker, iterate until pass or max iterations, present results.
</objective>

<execution_context>
@~/.claude/phaseflow/workflows/plan-phase.md
...
- `--reviews` — Replan incorporating cross-AI review feedback from REVIEWS.md (produced by `/phase:review`)
