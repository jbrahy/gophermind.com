...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/.phase-surface.json`
(sibling to `~/.claude/.phase-profile`) and re-stages the active skills directory in place.
Skill dirs live at `~/.claude/skills/phase-*/`.
...
| `reset` | Delete `.phase-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
`phaseflow/bin/lib/surface.cjs`. Display:
...
```
Base profile:   standard  (from .phase-surface.json)
Install profile: standard  (from .phase-profile)
```
...
1. Check if `.phase-surface.json` exists.
2. Delete it.
...
`~/.claude/skills/phase-*/` because the `claude global` layout has `destSubpath =
...
Surface state is stored at `${RUNTIME_CONFIG_DIR}/.phase-surface.json`
(i.e. `~/.claude/.phase-surface.json`).
...
- Missing `surface.cjs` → prompt: "Run `npm i -g phaseflow` to reinstall PhaseFlow."

<execution_context>
Surface state file: `~/.claude/.phase-surface.json`
Install profile marker: `~/.claude/.phase-profile`
Skill dirs: `~/.claude/skills/phase-*/`
Engine module: `~/.claude/phaseflow/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/phaseflow/bin/lib/clusters.cjs`
</execution_context>
