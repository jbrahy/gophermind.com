---
name: phase:quick
description: Quick task with PhaseFlow guarantees (atomic commits, state tracking) but skip optional agents
...
---
<objective>
Execute small, ad-hoc tasks with PhaseFlow guarantees (atomic commits, STATE.md tracking).

Quick mode is the same system with a shorter path:
- Spawns phase-planner (quick mode) + phase-executor(s)
...
<execution_context>
@~/.claude/phaseflow/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  phase-sdk query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
─────────────────────────────────────
Resume with: /phase:quick resume {slug}
```
...
- Status fields read via `phase-sdk query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
